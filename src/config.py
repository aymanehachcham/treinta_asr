...
class ModelConfig:
    ...
    def _download_whisper_model(
        ...
        available_models = {name: checksum for name, checksum in config.general.available_whisper_models}
        ...
        if hasher.hexdigest() != available_models[model_name]:
            logging.error(f'Checksum of the downloaded file does not match the expected value. \n')
    ...
    def _load_model(
        ...
        with open(full_config_path, 'r') as f:
            ...
            _model = Whisper.from_params(
                model_name=full_model_path,
                params=WhisperSerial(**model_config).config
            )
        return _model
